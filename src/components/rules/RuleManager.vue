...
<template>
  <div class="p-6 pb-20 text-white">
    <h1 class="text-3xl font-extrabold flex items-center mb-6 border-b border-slate-700 pb-4 neon-glow">
      <Shield class="w-8 h-8 mr-3 text-indigo-500" /> Editor de Regras No-Code
    </h1>
    
    <div class="grid gap-6">
      <div v-for="rule in rules" :key="rule.id" class="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-lg animate-slide-up">
        ...
        <div v-if="editingId === rule.id" class="space-y-4">
          ...
          <input v-model="tempRule.nome" class="w-full p-3 rounded-lg bg-slate-900 border border-slate-600 text-white font-bold mb-4 focus:ring-2 focus:ring-indigo-500 outline-none" />
          
          <!-- Construtor de Condições Visual -->
          <div class="bg-slate-900/50 p-4 rounded-lg border border-slate-600">
              ...
              <div v-for="(cond, idx) in tempRule.condicoes" :key="idx" class="flex gap-2 mb-2 items-center animate-fade-in">
                  <input v-model="cond.fato" placeholder="Campo (ex: tempo_servico)" class="flex-1 p-2 rounded bg-slate-800 border border-slate-600 text-xs text-white focus:border-indigo-500 outline-none" />
                  <select v-model="cond.operador" class="w-24 p-2 rounded bg-slate-800 border border-slate-600 text-xs text-white focus:border-indigo-500 outline-none">
                      <option v-for="op in OPERATORS" :key="op" :value="op">{{ op }}</option>
                  </select>
                  <input v-model="cond.valor" placeholder="Valor" class="flex-1 p-2 rounded bg-slate-800 border border-slate-600 text-xs text-white focus:border-indigo-500 outline-none" />
          ...
          <!-- Ação -->
          <div class="bg-slate-900/50 p-4 rounded-lg border border-slate-600">
              <h3 class="text-sm font-bold text-indigo-300 mb-2">Então (Resultado)</h3>
              <div class="flex flex-col md:flex-row gap-4">
                  <div class="w-full md:w-1/3">
                      <label class="text-xs text-slate-500 block mb-1">Status Final</label>
                      <select v-model="tempRule.acao_se_verdadeiro.status" class="w-full p-2 rounded bg-slate-800 border border-slate-600 text-white focus:border-indigo-500 outline-none">
                  ...
                  <div class="w-full md:w-2/3">
                      ...
                      <input v-model="tempRule.acao_se_verdadeiro.mensagem" class="w-full p-2 rounded bg-slate-800 border border-slate-600 text-white focus:border-indigo-500 outline-none" />
                  </div>
              </div>
          </div>

          <div class="flex justify-end space-x-3 mt-4 border-t border-slate-700 pt-4">
             <button @click="cancelEdit" class="px-4 py-2 bg-slate-700 rounded-lg hover:bg-slate-600 text-sm font-medium text-white transition-colors flex items-center">
            ...
            <div class="mt-3 space-y-2">
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex-1 flex flex-wrap gap-2">
                        <div v-for="(c, i) in rule.condicoes" :key="i" class="text-xs flex items-center bg-slate-900 border border-slate-700 rounded px-2 py-1">
                            <span class="text-indigo-300 font-mono mr-1">{{ c.fato }}</span>
                            <span class="text-yellow-500 font-bold mr-1">{{ c.operador }}</span>
                            <span class="text-white font-semibold">{{ c.valor }}</span>
                        </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2 bg-slate-900/50 border border-slate-700/50 rounded px-2 py-1">
